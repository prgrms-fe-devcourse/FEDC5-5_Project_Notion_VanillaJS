...
export function mdToHtml(mdText) {
  ...
  mdText = mdText.split("\n").map((line) => {
    let result = line
      .replace(H1_REGEX, makeHtmlTag("h1", "$1", style))
      .replace(H2_REGEX, makeHtmlTag("h2", "$1", style))
      .replace(H3_REGEX, makeHtmlTag("h3", "$1", style))
      .replace(BOLD_REGEX, makeHtmlTag("b", "$1", style))
      .replace(ITALIC_REGEX, makeHtmlTag("i", "$1", style))
      .replace(LIST_REGEX, makeHtmlTag("li", "$1", style))
      .replace(LINK_REGEX, makeHtmlTag("a", "$1", "href=$2"));
